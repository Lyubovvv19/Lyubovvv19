...
def rsp_date(request):
    groups, teachers = Pars()
        # print(group_dict)
    return render(request, 'mainn/rsp_date.html', {'groups': groups, 'teachers': teachers})
class Rasp:
...
def groupp(request):
    if request.method == 'POST':
        ...
        start = request.POST.get('calendar')
        ...
        return render(request, 'mainn/groupp.html', {'shedule': shedule, 'start': start, 'finish': finish, 'wiks': wiks, 'lst': lst, 'groups': groups, 'group_count': group_count, 'teachers': teachers, 'width': width})
    else:
        groups, teachers = Pars()
        # print(group_dict)
        return render(request, 'mainn/rsp_date.html', {'groups': groups, 'teachers': teachers})
...
def teacherr(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'mainn/teacherr.html',
