...
class Raspisanie(models.Model):
    weekday = models.CharField(max_length=12, blank=True, null=True)
    date = models.DateField(blank=True, null=True)
    teacher_name = models.CharField(max_length=100, blank=True, null=True)
    subject = models.CharField(max_length=121, blank=True, null=True)
    type_subject = models.CharField(max_length=5, blank=True, null=True)
    auditory_name = models.CharField(max_length=50, blank=True, null=True)
    ...
    def __str__(self):
        a = [self.subject, self.group_name, self.auditory_name, self.time, str(self.date)]
        return " ".join(a)
    class Meta:
        managed = False
        db_table = 'raspisanie'
    ...
    def get_date_time(self, date, time):
        if self.time == time and self.date == date:
            return self
        return ''
    # def get_date(self, item_name):
    #     if self.date == item_name:
    #         return self
    #     return ''
